...
contract YieldOptimizer is Ownable, ReentrancyGuard {
    ...
    function addStrategy(address strategy) external onlyOwner {
        ...
        require(
            strategyMetrics[strategy].strategy == address(0),
        ...
        returns (address[] memory strategyList, uint256[] memory allocations)
    {
        ...
        for (uint256 i = 0; i < strategies.length; i++) {
            if (
                strategyMetrics[strategies[i]].apy > 0 &&
        ...
        for (uint256 i = 0; i < activeStrategies; i++) {
            ...
            uint256 maxAllocation = (totalAmount *
                config.maxSingleStrategyAllocation) / 10000;
    ...
    function shouldRebalance(
        ...
        external
        view
        returns (bool needsRebalance, uint256[] memory recommendedAllocations)
    {
        require(
            strategyList.length == currentAllocations.length,
    ...
    function getBestStrategy()
        external
        view
        returns (address strategy, uint256 apy)
    {
        uint256 bestScore = 0;
    ...
    function updateConfig(
        ...
        require(
            newConfig.maxSingleStrategyAllocation <= 10000,
            "Allocation too high"
        ); // Max 100%
    ...
    function _updateReliabilityScore(
        address strategy,
        uint256 currentAPY
    ) internal {
        ...
        uint256 newReliability = variance < 100
            ? 9000 // Very stable
            : variance < 500
                ? 7000 // Moderately stable
                : variance < 1000
                    ? 5000 // Average stability
                    : 3000; // Volatile

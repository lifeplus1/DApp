...
contract MockLPToken is ERC20, Ownable {
    ...
    function setMinter(address m, bool a) external onlyOwner { minters[m] = a; }
    modifier onlyMinter() { require(minters[msg.sender] || msg.sender == owner(), "NOT_MINTER"); _; }
...
contract MockCurvePool is Ownable {
    ...
    constructor(address _dai, address _usdc, address _usdt, address _lp) Ownable(msg.sender) {
    ...
    function coins(uint256 i) external view returns(address) { return coinsArr[i]; }
    function fee() external view returns(uint256) { return poolFee; }
}
...
contract MockCurveGauge is Ownable {
    ...
    function balanceOf(address user) external view returns(uint256) { return staked[user]; }
    function claimable_tokens(address user) external view returns(uint256) { return pending[user]; }

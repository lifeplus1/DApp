...
contract PortfolioManager is Ownable, ReentrancyGuard, Pausable {
    ...
    modifier gasLimitCheck() {
        ...
        if (gasUsed > MAX_GAS_PER_REBALANCE) {
            emit GasConsumptionWarning("rebalance", gasUsed, MAX_GAS_PER_REBALANCE);
        }
    }
    ...
    modifier emergencyCooldown() {
        require(
            block.timestamp >= lastEmergencyAction[msg.sender] + EMERGENCY_COOLDOWN,
    ...
    function addStrategy(
        ...
        require(targetAllocationBPS > 0 && targetAllocationBPS <= MAX_ALLOCATION_BPS, "Invalid allocation");
        ...
        emit SecurityEvent("strategy_added", strategy, abi.encode(targetAllocationBPS, strategyName));
    }
        ...
        gasLimitCheck 
    {
        ...
        emit PortfolioRebalanced(totalPortfolioValue, gasUsageHistory[msg.sender], block.timestamp);
        emit PerformanceMetric("portfolio_value", totalPortfolioValue, block.timestamp);
    }
     ...
     */
    function emergencyPauseStrategy(address strategy) 
        external 
        ...
        emergencyCooldown 
    {
        ...
        emit SecurityEvent("strategy_paused", strategy, abi.encode(msg.sender, block.timestamp));
    }
    ...
    globalEmergencyPaused = true;
        ...
        emit SecurityEvent("global_pause", msg.sender, abi.encode(block.timestamp));
    }
    ...
    globalEmergencyPaused = false;
        ...
        emit SecurityEvent("operations_resumed", msg.sender, abi.encode(block.timestamp));
    }
    ...
    function getTotalPortfolioValue() public view returns (uint256 totalValue) {
        ...
        for (uint256 i = 0; i < strategiesLength;) {
            if (strategies[activeStrategies[i]].isActive) {
                totalValue += IStrategyV2(activeStrategies[i]).balanceOf(address(this));
            }
            unchecked { ++i; }
        }
    ...
    function calculateWeightedAPY() public view returns (uint256 weightedAPY) {
        ...
        for (uint256 i = 0; i < strategiesLength;) {
            ...
            if (strategies[strategy].isActive) {
                uint256 balance = IStrategyV2(strategy).balanceOf(address(this));

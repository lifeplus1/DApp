    ...
    Pausable
{
        ...
        returns (uint256 shares)
    {
        ...
        // Supply to Aave
        _asset.forceApprove(address(aavePool), assets);
        ...
        )
    {
        return (
            "AaveV3Strategy",
            "1.0.0",
            "Aave V3 lending strategy with leverage support"
        );
    }
    ...
    function getAPY() external view returns (uint256) {
        (, uint128 liquidityRate, , , , , , , , , , , , , ) = aavePool
        ...
        returns (uint256 yield)
    {
        ...
        yield = currentBalance > totalDeposits
    ...
    function emergencyWithdraw() external onlyRole(EMERGENCY_ROLE) {
        ...
        if (debt > 0) {
            ...
            _asset.forceApprove(address(aavePool), debt);
            aavePool.repay(address(_asset), debt, 2, address(this)); // 2 = variable rate
        }
        ...
        // Transfer all USDC to the emergency caller (who should be trusted)
    ...
    function _adjustLeverage() internal {
        ...
        if (currentHealthFactor > TARGET_HEALTH_FACTOR + LIQUIDATION_BUFFER) {
            ...
            currentHealthFactor < TARGET_HEALTH_FACTOR - LIQUIDATION_BUFFER
        ) {
    ...
    function _calculateMaxBorrow() internal view returns (uint256) {
        (, , uint256 availableBorrows, , , ) = aavePool.getUserAccountData(
            address(this)
        );
        uint256 currentLTV = _getCurrentLTV();

        if (currentLTV < config.targetLTV) {
            uint256 maxAdditionalBorrow = (totalDeposits *
                (config.targetLTV - currentLTV)) / MAX_BPS;
    ...
    function _increaseLeverage(uint256 borrowAmount) internal {
        // Borrow USDC
        aavePool.borrow(address(_asset), borrowAmount, 2, 0, address(this)); // 2 = variable rate
        totalBorrows += borrowAmount;

        // Supply borrowed USDC back to Aave
        _asset.forceApprove(address(aavePool), borrowAmount);
        aavePool.supply(address(_asset), borrowAmount, address(this), 0);
    ...
    function _reduceLeverage(uint256 repayAmount) internal {
        ...
        if (availableBalance < repayAmount) {
            ...
            aavePool.withdraw(address(_asset), toWithdraw, address(this));
            totalDeposits = totalDeposits > toWithdraw
                ? totalDeposits - toWithdraw
                : 0;
        }

        // Repay debt
        _asset.forceApprove(address(aavePool), repayAmount);
        uint256 actualRepay = aavePool.repay(
            address(_asset),
            repayAmount,
            2,
            address(this)
        );
        totalBorrows = totalBorrows > actualRepay
    ...
    function _getAvailableLiquidity() internal view returns (uint256) {
        return
            aToken.balanceOf(address(this)) + _asset.balanceOf(address(this));
    }
        ...
        )
    {
        ...
        leverageRatio = totalDeposits > 0
            ? (totalBorrows * MAX_BPS) / totalDeposits
            : 0;

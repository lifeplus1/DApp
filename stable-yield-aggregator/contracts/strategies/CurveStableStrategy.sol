...
contract CurveStableStrategy is IStrategyV2, Ownable, ReentrancyGuard {
    ...
    event FeeControllerUpdated(address newFeeController);
    ...
    constructor(
        ...
        IERC20(_asset).approve(_curvePool, type(uint256).max);
    ...
    function deposit(
        ...
        uint256 lpTokensReceived = curvePool.add_liquidity(
            amounts,
            minLPTokens
        );
    ...
    function withdraw(
        ...
        _asset.transfer(receiver, usdcReceived);
    ...
    function harvest() external override returns (uint256 yield) {
        ...
        if (crvBalance > 0) {
            ...
            // Send fees to fee controller if configured
            if (feeController != address(0)) {
                uint256 feeAmount = (crvBalance * performanceFee) / 10000;
                if (feeAmount > 0) {
                    crvToken.transfer(feeController, feeAmount);
                    // Note: In a real implementation, the fee controller would need to handle CRV tokens
                    // or we would convert CRV to USDC first and then send USDC fees
                }
            }
    ...
    function totalAssets() external view override returns (uint256) {
        ...
        return usdcValue + pendingCRVValue + _asset.balanceOf(address(this));
    }
    ...
    function emergencyWithdraw(address recipient) external onlyOwner {
        ...
        if (usdcBalance > 0) {
            _asset.transfer(recipient, usdcBalance);
        }
    ...
    /**
     * @notice Set the fee controller address (owner only)
     * @param _feeController Address of the fee controller contract
     */
    function setFeeController(address _feeController) external onlyOwner {
        feeController = _feeController;
        emit FeeControllerUpdated(_feeController);
    }

    // Internal calculation functions

    function _calculateMinLPTokens(
        uint256 usdcAmount
    ...
    function _calculateMinUSDCFromLP(
        uint256 lpTokens
    ) internal view returns (uint256) {
    ...
    function _calculateUSDCValueFromLP(
        uint256 lpTokens
    ) internal view returns (uint256) {
    ...
    function _convertCRVToUSDCValue(
        uint256 crvAmount

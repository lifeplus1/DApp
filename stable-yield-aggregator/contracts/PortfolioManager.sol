...
contract PortfolioManager is Ownable, ReentrancyGuard {
    ...
    // Events
    event StrategyAdded(address indexed strategy, uint256 targetAllocation, string name);
    event StrategyRemoved(address indexed strategy);
    event StrategyUpdated(address indexed strategy, uint256 newTargetAllocation);
    event PortfolioRebalanced(uint256 totalValue, uint256 gasUsed, uint256 timestamp);
    ...
    modifier strategyExists(address strategy) {
        require(strategies[strategy].strategyAddress != address(0), "Strategy not found");
        _;
    }
    ...
    function addStrategy(
        ...
        require(targetAllocationBPS > 0 && targetAllocationBPS <= 10000, "Invalid allocation");
        require(bytes(strategyName).length > 0, "Strategy name required");
        
        // Validate total allocation doesn't exceed 100%
        uint256 totalAllocation = _calculateTotalTargetAllocation() + targetAllocationBPS;
    ...
    function updateStrategyAllocation(
        ...
        require(newTargetAllocationBPS > 0 && newTargetAllocationBPS <= 10000, "Invalid allocation");
     ...
     */
    function removeStrategy(address strategy) external onlyOwner strategyExists(strategy) {
    ...
    function rebalancePortfolio() external onlyRebalancer nonReentrant returns (uint256 gasUsed) {
        ...
        uint256[] memory targetBalances = new uint256[](activeStrategies.length);
        uint256[] memory currentBalances = new uint256[](activeStrategies.length);
        
        for (uint256 i = 0; i < activeStrategies.length; i++) {
            address strategy = activeStrategies[i];
            if (strategies[strategy].isActive && !strategies[strategy].isEmergencyPaused) {
                targetBalances[i] = (totalPortfolioValue * strategies[strategy].targetAllocationBPS) / 10000;
                currentBalances[i] = IStrategyV2(strategy).balanceOf(address(this));
            }
        }
     ...
     */
    function calculateOptimalAllocation() external view returns (uint256[] memory optimalAllocations) {
        ...
        if (totalAPYWeight > 0) {
            for (uint256 i = 0; i < activeStrategies.length; i++) {
                if (strategyAPYs[i] > 0) {
                    optimalAllocations[i] = (strategyAPYs[i] * 10000) / totalAPYWeight;
                }
            }
        }
     ...
     */
    function harvestAllStrategies() external onlyRebalancer returns (uint256 totalYield) {
        for (uint256 i = 0; i < activeStrategies.length; i++) {
            address strategy = activeStrategies[i];
            if (strategies[strategy].isActive && !strategies[strategy].isEmergencyPaused) {
     ...
     */
    function emergencyPauseStrategy(address strategy) external onlyEmergencyOperator strategyExists(strategy) {
     ...
     */
    function getActiveStrategies() external view returns (address[] memory strategiesList) {
    ...
    function needsRebalancing() external view returns (bool needsRebalance) {
        ...
        for (uint256 i = 0; i < activeStrategies.length; i++) {
            address strategy = activeStrategies[i];
            if (strategies[strategy].isActive && !strategies[strategy].isEmergencyPaused) {
                uint256 currentBalance = IStrategyV2(strategy).balanceOf(address(this));
                uint256 targetBalance = (totalValue * strategies[strategy].targetAllocationBPS) / 10000;
                
                uint256 deviation = currentBalance > targetBalance ? 
                    currentBalance - targetBalance : 
                    targetBalance - currentBalance;
                
                uint256 deviationBPS = totalValue > 0 ? (deviation * 10000) / totalValue : 0;
    ...
    function _calculateTotalTargetAllocation() internal view returns (uint256 total) {
        for (uint256 i = 0; i < activeStrategies.length; i++) {
    ...
    function _executeRebalance(
        ...
        for (uint256 i = 0; i < activeStrategies.length; i++) {
            address strategy = activeStrategies[i];
            if (!strategies[strategy].isActive || strategies[strategy].isEmergencyPaused) continue;
            ...
            if (target > current) {
                ...
                IStrategyV2(strategy).withdraw(withdrawAmount, address(this), address(this));
            }
            
            strategies[strategy].currentBalance = IStrategyV2(strategy).balanceOf(address(this));
            strategies[strategy].lastRebalanceTime = block.timestamp;
        }
    }
    
    function _triggerRebalanceIfNeeded() internal {

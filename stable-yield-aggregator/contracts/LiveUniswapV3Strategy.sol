...
contract LiveUniswapV3Strategy is IStrategyV2, Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 amount, address user) external nonReentrant returns (uint256 shares) {
    ...
    function withdraw(uint256 shares, address receiver, address owner) external nonReentrant returns (uint256 amount) {
        require(shares <= userShares[owner], "Insufficient shares");
        require(shares > 0, "Shares must be > 0");
        require(msg.sender == owner || msg.sender == address(this), "Unauthorized"); // Allow vault to call
    ...
    function getHistoricalAPY() external view returns (uint256[] memory apys, uint256[] memory timestamps) {
        return (historicalAPY, apyTimestamps);
    }
    
    function getPositionInfo() external view returns (uint256 positionId, uint128 liquidity, uint256 feesCollected) {
    ...
    function _collectFees() internal {
        ...
        uint256 totalValue = token.balanceOf(address(this)) + _getPositionValue();
        uint256 simulatedFees = (totalValue * BASE_YIELD_BPS * timeElapsed) / (365 days * 10000);
    ...
    function _generatePositionId() internal view returns (uint256 id) {
        return uint256(keccak256(abi.encodePacked(block.timestamp, address(this), msg.sender))) % 1000000 + 1;
    }
